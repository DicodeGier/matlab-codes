...
counter = 1;
for budget = 1000:1000:max_x
    [mean_path,~,~,upgrades_to_shsr,upgrades_to_hsr,~,~] = one_shortest_path_mean_path(budget,0,nodes,rhs_till_now,obj,sparse_constraint_matrix,number_edges,cost_shsr,cost_hsr);
    y(counter) = mean_path;
...
%load('one_shortest_path_mean_path.mat')
...
ax.XAxis.Exponent = 0;
xtickformat('%.0f');
xticks(0:20000:max_x)
title('Mean path per budget')
xlabel('Budget in millions of €')
ylabel('Mean path in hours')
grid on
legend('Mean path')
