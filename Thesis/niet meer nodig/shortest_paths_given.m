...
z_first_constraint = ones(number_final_routes,1);
...
model.rhs = rhs;
model.sense = [repmat('>',number_final_routes,1); repmat('<',number_edges,1); repmat('<',number_edges,1); repmat('<', number_edges,1); '<'];
...
fprintf('\n')
fprintf('-----------------------\n')
fprintf('|arcs upgraded to shsr|\n')
fprintf('-----------------------\n')
for i = 1:nodes
    for j = 1:nodes
        if upgrade_matrix_shsr(i,j) == 1
            city1 = cell2mat(cities(i));
            city2 = cell2mat(cities(j));
            fprintf('   ')
            fprintf(city1)
            fprintf('-')
            fprintf(city2)
...
fprintf('----------------------\n')
fprintf('|arcs upgraded to hsr|\n')
fprintf('----------------------\n')
for i = 1:nodes
    for j = 1:nodes
        if upgrade_matrix_hsr(i,j) == 1
            city1 = cell2mat(cities(i));
            city2 = cell2mat(cities(j));
            fprintf('   ')
            fprintf(city1)
...
cost_model.sense = [repmat('>',number_final_routes,1); repmat('<',number_edges,1); repmat('<',number_edges,1); repmat('<', number_edges,1); '<';'<'];
...
fprintf('By increasing the maximum shortest path with %g percent to %g hours, we need €%g instead of €%g\n',percentual_increase, cost_solution.x(1),cost_solution.objval,sum(transpose(upgrades_shsr).*cost_shsr + transpose(upgrades_hsr).*cost_hsr))
...
for i = 1:nodes
    for j = 1:nodes
        if cost_upgrade_matrix_shsr(i,j) == 1
            city1 = cell2mat(cities(i));
            city2 = cell2mat(cities(j));
            fprintf('   ')
            fprintf(city1)
...
for i = 1:nodes
    for j = 1:nodes
        if cost_upgrade_matrix_hsr(i,j) == 1
            city1 = cell2mat(cities(i));
            city2 = cell2mat(cities(j));
            fprintf('   ')
            fprintf(city1)
