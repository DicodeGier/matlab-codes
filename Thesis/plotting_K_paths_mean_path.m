...
optimal_values_2 = zeros(1,length(budgets));
total_upgrades_shsr_2 = zeros(size(hours,1));
total_upgrades_hsr_2 = zeros(size(hours,1));
used_budgets_2 = zeros(1,length(budgets));

for idx = 1:numel(budgets)
   budget = budgets(1,idx);
   disp(budget)
   [optimal_value, used_budget, upgrade_matrix_shsr, upgrade_matrix_hsr, ~] = K_paths_mean_path(budget, rhs_till_now, obj, sparse_constraint_matrix, number_routes, number_edges, binary_constraint_matrix, number_binary_variables, cost_shsr, cost_hsr, all_K);
   optimal_values_2(1,idx) = optimal_value;
   total_upgrades_shsr_2 = total_upgrades_shsr_2 + upgrade_matrix_shsr;
   total_upgrades_hsr_2 = total_upgrades_hsr_2 + upgrade_matrix_hsr;
   used_budgets_2(1,idx) = used_budget;
end