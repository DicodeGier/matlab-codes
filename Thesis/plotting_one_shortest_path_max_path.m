...
x = [300000];
y = zeros(1,length(x));
mean_paths = zeros(1,length(x));

total_upgrades_to_shsr = zeros(nodes);
total_upgrades_to_hsr = zeros(nodes);

counter = 1;
for budget = 300000
    [max_shortest_path,~,~,upgrades_to_shsr,upgrades_to_hsr,~,~] = one_shortest_path_shortened(budget,0,nodes,rhs_till_now,obj,sparse_constraint_matrix,number_routes,number_edges,cost_shsr,cost_hsr);
    y(counter) = max_shortest_path;
    
    new_travel_times = hours - red_hsr_matrix.*upgrades_to_hsr - red_shsr_matrix.*upgrades_to_shsr;
    mean_path = 0;
    for j = 1:size(result,1)
        adjacency_matrix = result{j,5};
        length_route = sum(sum(adjacency_matrix.*new_travel_times));
        mean_path = mean_path + length_route;
    end
    mean_path = mean_path/size(result,1);
    mean_paths(counter) = mean_path;
    total_upgrades_to_shsr = total_upgrades_to_shsr + upgrades_to_shsr;
    total_upgrades_to_hsr = total_upgrades_to_hsr + upgrades_to_hsr;
    counter = counter+1;
end

%load('one_shortest_path_max_path.mat')
close all
fig = plot(x,y);
...
cities{44} =    'Thessaloniki';
 ...
 total_edges_shsr = sum(sum(total_upgrades_to_shsr > 0));
 cell_edges_shsr = cell(total_edges_shsr,3);
 counter = 1;
 for i = 1:49
     for j = 1:49
     if total_upgrades_to_shsr(i,j) > 0
        cell_edges_shsr{counter,1} = cities{i};
        cell_edges_shsr{counter,2} = cities{j};
        cell_edges_shsr{counter,3} = total_upgrades_to_shsr(i,j);
        counter = counter + 1;
     end
     end
 end
 ...
 total_edges_hsr = sum(sum(total_upgrades_to_hsr > 0));
 cell_edges_hsr = cell(total_edges_hsr,3);
 counter = 1;
 for i = 1:49
     for j = 1:49
     if total_upgrades_to_hsr(i,j) > 0
        cell_edges_hsr{counter,1} = cities{i};
        cell_edges_hsr{counter,2} = cities{j};
        cell_edges_hsr{counter,3} = total_upgrades_to_hsr(i,j);
        counter = counter + 1;
     end
     end
 end
 ...
 load('K_paths_data_optimal_values.mat')
 comparison = [y; optimal_values];
 
 load('K_paths_4_paths_optimal_values')
 
 plot(x,y)
 hold on
 plot(x,optimal_values)
 
 legend('one path', 'four paths')
