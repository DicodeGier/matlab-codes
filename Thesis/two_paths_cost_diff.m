...
cost_km_shsr = 16.5;
cost_km_hsr = 25;
mountain_cost_shsr = 62;
mountain_cost_hsr = 75;
K = 2;
...
data = importfile('data network complete.xlsx', 'mountains');
mountains = data.data;
mountains(isnan(mountains)) = 0;
clear data
mountains(~indices) = Inf;
mountains = mountains';
mountains = mountains(:);
mountains(mountains == Inf) = [];
mountains = mountains';

cost_shsr = cost_km_shsr.*distances.*u_shsr + mountain_cost_shsr * mountains;
cost_hsr = cost_km_hsr.*distances.*u_hsr + mountain_cost_hsr * mountains;
...
optimal_values = zeros(1,length(budgets));
total_upgrades_shsr = zeros(size(hours,1));
total_upgrades_hsr = zeros(size(hours,1));
used_budgets = zeros(1,length(budgets));
all_routes_selected_total = zeros(length(all_K),numel(budgets));
tic
for idx = 1:numel(budgets)
   ...
   optimal_values(1,idx) = optimal_value;
   total_upgrades_shsr = total_upgrades_shsr + upgrade_matrix_shsr;
   total_upgrades_hsr = total_upgrades_hsr + upgrade_matrix_hsr;
   used_budgets(1,idx) = used_budget;
   all_routes_selected_total(:,idx) = all_routes_selected;
end
toc
...
cities{44} =    'Thessaloniki';
 ...
 total_edges_shsr = sum(sum(total_upgrades_shsr > 0));
 cell_edges_shsr = cell(total_edges_shsr,3);
 counter = 1;
 for i = 1:49
     for j = 1:49
     if total_upgrades_shsr(i,j) > 0
        cell_edges_shsr{counter,1} = cities{i};
        cell_edges_shsr{counter,2} = cities{j};
        cell_edges_shsr{counter,3} = total_upgrades_shsr(i,j);
        counter = counter + 1;
     end
     end
 end
 ...
 total_edges_hsr = sum(sum(total_upgrades_hsr > 0));
 cell_edges_hsr = cell(total_edges_hsr,3);
 counter = 1;
 for i = 1:49
     for j = 1:49
     if total_upgrades_hsr(i,j) > 0
        cell_edges_hsr{counter,1} = cities{i};
        cell_edges_hsr{counter,2} = cities{j};
        cell_edges_hsr{counter,3} = total_upgrades_hsr(i,j);
        counter = counter + 1;
     end
     end
 end
