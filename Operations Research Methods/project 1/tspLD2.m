function [lambda, zld, zlr] = tspLD2(C, K2, K3, E, w)
...
constraint_matrix = sparse([first_constraint;second_constraint]);
...
for i = 1:E
    ...
    dualized_constraint_matrix = [dualized_constraint_matrix;sparse(transpose(constraint_to_add_4))];
...
%with lambda
main_model.A = constraint_matrix;
main_model.rhs = rhs;
...
model_integer.vtype = repmat('B',n*n,1);
...
model_con.lb = zeros(n*n,1);
model_con.ub = ones(n*n,1);
model_con.vtype = repmat('C',n*n,1);
...
function constraint_matrix = constraint_generator(constraint_matrix,n,loop_vec,k,size_set)
    ...
    for i = 1:k
        ...
        constraint_matrix = [constraint_matrix;sparse(transpose(constraint_to_add))];
    end
