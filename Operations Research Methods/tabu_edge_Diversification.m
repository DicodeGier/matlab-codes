%this version tries out more difficult techniques to arrive at a better
%solution, but optimal solution found is generally higher than the one
%from the basic tabu search file
...
function [tour,cost,it,tabu_matrix] = iterative_TSP_edge(A,starting_vertex)
    ...
    while it < 1000
        ...
        [new_tour,new_cost,best_a,best_b,best_c,best_d,times_increasing,tabu_check] = two_edge_exchange_neighborhood(A,tour,cost,tabu_matrix,global_optimum,times_increasing,step_increaser);
        div_it = div_it + 1;
        ...
        %if new_cost < cost
            ...
            elseif best_a ~= 0
                ...
                %{
                if tabu_check == 1
                    tabu_increaser = tabu_increaser+30;
                end
                %}
                ...
                if mod(it,100) == 0
                    fprintf('iteration: %g with global optimum %g\n',it,global_optimum)
                end
                %{
                if new_cost < cost
...
function [best_tour,best_cost,best_a,best_b,best_c,best_d,times_increasing,tabu_check] = two_edge_exchange_neighborhood(A,tour,cost,tabu_matrix,global_optimum,times_increasing,step_increaser)
    N = size(A,2);
    ...
    if times_increasing > 0
      ...
      [best_tour,best_cost,best_a,best_b,best_c,best_d,tabu_check] = two_edge_exchange_neigborhood_increase(A,best_tour,best_cost,tabu_matrix);  
    else
...
function [best_tour,best_cost_upper,best_a,best_b,best_c,best_d,tabu_check] = two_edge_exchange_neigborhood_increase(A,tour,cost,tabu_matrix)
    N = size(A,2);
    tabu_check = 0;
    best_cost = cost; % original cost
    best_cost_upper = Inf;
