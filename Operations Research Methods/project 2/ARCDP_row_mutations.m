...
l = 800; % size tournament mating pool
mating = 500; % size mating pool
l2 = 3; % size tournament parent selection
maxit = 400; % stopping criterion: maximum of 1000 iterations
sameit = 50; % stopping criterion: stop if after sameit iterations global opt has not changed
T = 10; % stopping criterion: time
mutation_prob = 0.2;

[best_cost,global_opt,all_k,all_w,B,S,k,w] = ARCDP2(M,K_max,W_max,P,mating,l,l2,maxit,sameit,T,mutation_prob);


function [best_cost,global_opt,all_k,all_w,B,S,k,w] = ARCDP2(A,K_max,W_max,P,mating,l,l2,maxit,sameit,T,mutation_prob)
    ...
    fprintf('--------------------------\n')
    fprintf('\n')
    while it < maxit && stop == 1 && global_opt ~= 0 && totaltime < T %print == 1 is stopping criterion for sameit: if it > sameit => print = 0 so the new iteration will not be printed in the command window
        tstart = tic;
        it = it+1;
        [mating_pool,mating_pool_obj_values] = mating_pool_creater(objective_values,mating,l);
...
function [children,objective_values] = offspring_creation(A,initial_solution,P,parents,K_max,W_max,mutation_prob)
    ...
    for row = 1:round(P/2) 
        ...
        % Generate an average weight matrix to use for the offspring
...
function parents = parents_selection(mating_pool,mating_pool_obj_values,P,l2)
 ...
 function [mating_pool,mating_pool_obj_values] = mating_pool_creater(objective_values,mating,l)
