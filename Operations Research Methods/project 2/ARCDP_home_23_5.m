...
P = 1500; % # initial solutions
l = 750; % size tournament mating pool
M_size = 100; % size mating pool
l2 = 50; % size tournament parent selection
maxit = 500; % stopping criterion: maximum of 1000 iterations
sameit = 50; % stopping criterion: stop if after sameit iterations global opt has not changed
T = 300; % stopping criterion: time
mutation_prob = 0.2;

[best_cost,global_opt,all_k,all_w,B,S,k,w] = ARCDP2(M,K_max,W_max,P,M_size,l,l2,maxit,sameit,T,mutation_prob);


function [best_cost,global_opt,all_k,all_w,B,S,k,w] = ARCDP2(A,K_max,W_max,P,M_size,l,l2,maxit,sameit,T,mutation_prob)
    ...
    fprintf('--------------------------\n')
    fprintf('\n')
    while it < maxit && global_opt ~= 0 && totaltime < T %print == 1 is stopping criterion for sameit: if it > sameit => print = 0 so the new iteration will not be printed in the command window
        tstart = tic;
        it = it+1;
...
function [children,objective_values] = offspring_creation(A,initial_solution,P,parents,K_max,W_max,mutation_prob)
    ...
    for row = 1:round(P/2) 
        ...
        weight_vector1 = weight_vector1(mod(0:length(weight_vector2)-1,length(weight_vector1))+1);
        new_weight = (weight_vector1 + weight_vector2)./2;
        beta = 0;
        for i = 1:length(new_weight)
            if mod(new_weight(i),1) ~= 0
               if beta == 0
                   new_weight(i) = round(new_weight(i)) - 1;
                   beta = 1;
               elseif beta == 1
                   new_weight(i) = round(new_weight(i));
                   beta = 0;
               end
            end
        end
        ...
        for i = 1:max_matrices
            ...
            if new_weight(i) < weight_left - max_matrices + i
                childweight = new_weight(i);
                weight_left = weight_left - childweight;
            else
                childweight = 1;
            end
            ...
            offspring{1,i} = childweight.*offspring_matrix1;
            offspring{2,i} = childweight.*offspring_matrix2;
           
            % Add found matrices to a sum 
            sum_child1 = sum_child1 + childweight.*offspring_matrix1;
            sum_child2 = sum_child2 + childweight.*offspring_matrix2;
