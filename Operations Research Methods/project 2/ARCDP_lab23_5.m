...
P = 250; % # initial solutions
l = 150; % size tournament mating pool
M_size = 200; % size mating pool
l2 = 100; % size tournament parent selection
maxit = 5000; % stopping criterion: maximum of 1000 iterations
sameit = 50; % stopping criterion: stop if after sameit iterations global opt has not changed
T = 130; % stopping criterion: time
mutation_prob = 0.2;
...
function [all_solutions,objective_values] = initial_solutions(A,K_max,W_max,P)
    ...
    number_matrices = repmat(K_max,P,1);%randi([1,K_max],P,1);
